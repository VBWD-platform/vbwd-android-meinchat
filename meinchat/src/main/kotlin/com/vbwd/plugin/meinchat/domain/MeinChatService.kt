...
class DefaultMeinChatService(private val api: ApiClient) : MeinChatService {
    ...
    override suspend fun sendText(
        ...
        body: String,
        meta: MessageMeta?,
    ): ChatMessage = api.post(MeinChatEndpoints.messages(convId), SendBody(body, meta))
    ...
    override suspend fun transferTokens(
        ...
        amount: Int,
        note: String?,
    ): TokenTransferResult = api.post(MeinChatEndpoints.TOKEN_TRANSFER, TransferBody(toNickname, amount, note))
    ...
    override suspend fun fetchRooms(): List<Room> = api.get<RoomsResponse>(MeinChatEndpoints.ROOMS).items ?: emptyList()

    override suspend fun fetchRoomMessages(
        roomId: String,
        before: String?,
        limit: Int,
    ): List<ChatMessage> = api.get<MessagesResponse>(MeinChatEndpoints.roomMessages(roomId)).items ?: emptyList()

    override suspend fun sendRoomText(
        roomId: String,
        body: String,
        meta: MessageMeta?,
    ): ChatMessage = api.post(MeinChatEndpoints.roomMessages(roomId), SendBody(body, meta))
    ...
    @Serializable private data class OpenBody(
